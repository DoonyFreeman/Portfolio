...
/* End */
;
; /* Start:"a:4:{s:4:"full";s:62:"/local/templates/masterstroy/assets/js/home.js?178670439216433";s:6:"source";s:46:"/local/templates/masterstroy/assets/js/home.js";s:3:"min";s:0:"";s:3:"map";s:0:"";}"*/
(function () {
    'use strict';
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (hero) {
            ...
            // Лента упирается в края, а не заворачивается в кольцо: в макете стрелка
            // «назад» нарисована неактивной на 40% при счётчике 01, то есть с первого
            // слайда назад идти некуда (компонент-сеты 0:1187 / 0:1982, issue #223).
            function showSlide(index) {
                ...
                window.msArrowEdgesByIndex(heroPrev, heroNext, current, slides.length);
            }

            heroPrev.addEventListener('click', function () { showSlide(current - 1); });
            heroNext.addEventListener('click', function () { showSlide(current + 1); });
            showSlide(0);
        }

        var comfortRoot = document.querySelector('[data-home-comfort-root]');
        if (comfortRoot) {
            var comfortGalleries = Array.prototype.slice.call(comfortRoot.querySelectorAll('[data-home-comfort-gallery]'));
            var comfortCurrent = 0;
            var comfortCounter = comfortRoot.querySelector('[data-home-comfort-current]');
            var comfortTotal = comfortRoot.querySelector('[data-home-comfort-total]');
            var comfortPrev = comfortRoot.querySelector('[data-home-comfort-prev]');
            var comfortNext = comfortRoot.querySelector('[data-home-comfort-next]');
            var comfortDesktop = window.matchMedia('(min-width: 1280px)');

            function activeComfortGallery() {
                return comfortRoot.querySelector('[data-home-comfort-gallery="' + (comfortDesktop.matches ? 'desktop' : 'compact') + '"]');
            }

            function showComfortSlide(index) {
                var gallery = activeComfortGallery();
                if (!gallery) return;
                var slides = Array.prototype.slice.call(gallery.querySelectorAll('[data-home-comfort-slide]'));
                comfortCurrent = Math.max(0, Math.min(slides.length - 1, index));

                comfortGalleries.forEach(function (item) {
                    var isActiveGallery = item === gallery;
                    item.setAttribute('aria-hidden', isActiveGallery ? 'false' : 'true');
                    Array.prototype.forEach.call(item.querySelectorAll('[data-home-comfort-slide]'), function (slide, slideIndex) {
                        var active = isActiveGallery && slideIndex === comfortCurrent;
                        slide.classList.toggle('is-active', active);
                        slide.setAttribute('aria-hidden', active ? 'false' : 'true');
                    });
                });

                gallery.style.transform = 'translate3d(' + (-slides[comfortCurrent].offsetLeft) + 'px, 0, 0)';
                if (comfortCounter) comfortCounter.textContent = String(comfortCurrent + 1).padStart(2, '0');
                if (comfortTotal) comfortTotal.textContent = String(slides.length).padStart(2, '0');
                window.msArrowEdgesByIndex(comfortPrev, comfortNext, comfortCurrent, slides.length);
            }

            if (comfortPrev && comfortNext && comfortGalleries.length) {
                comfortPrev.addEventListener('click', function () { showComfortSlide(comfortCurrent - 1); });
                comfortNext.addEventListener('click', function () { showComfortSlide(comfortCurrent + 1); });
                comfortGalleries.forEach(function (gallery) {
                    var comfortPointerStart = null;
                    gallery.addEventListener('keydown', function (event) {
                        if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') {
                            event.preventDefault();
                            showComfortSlide(comfortCurrent + (event.key === 'ArrowRight' ? 1 : -1));
                        }
                    });
                    gallery.addEventListener('pointerdown', function (event) { comfortPointerStart = event.clientX; });
                    gallery.addEventListener('pointerup', function (event) {
                        if (comfortPointerStart === null) return;
                        var delta = event.clientX - comfortPointerStart;
                        if (Math.abs(delta) > 40) showComfortSlide(comfortCurrent + (delta < 0 ? 1 : -1));
                        comfortPointerStart = null;
                    });
                    gallery.addEventListener('pointercancel', function () { comfortPointerStart = null; });
                });
                var syncComfortLayout = function () {
                    window.requestAnimationFrame(function () { showComfortSlide(comfortCurrent); });
                };
                if (typeof comfortDesktop.addEventListener === 'function') comfortDesktop.addEventListener('change', syncComfortLayout);
                else comfortDesktop.addListener(syncComfortLayout);
                window.addEventListener('resize', syncComfortLayout);
                showComfortSlide(0);
            }
        }

        var catalogFilter = document.querySelector('[data-home-catalog-filter]');
        if (catalogFilter) {
            var catalogLayer = document.querySelector('[data-home-catalog-layer]');
            var catalogOpen = document.querySelector('[data-home-catalog-open]');
            var catalogClose = catalogLayer ? catalogLayer.querySelectorAll('[data-home-catalog-close]') : [];
            var catalogSwipe = catalogFilter.querySelector('[data-home-catalog-swipe]');
            var catalogMobile = window.matchMedia('(max-width: 767px)');
            var catalogLastFocus = null;
            var catalogStartY = 0;
            var catalogDragging = false;

            function catalogFocusable() {
                return Array.prototype.slice.call(catalogFilter.querySelectorAll('button,select,input,a[href],[tabindex]:not([tabindex="-1"])')).filter(function (element) {
                    return !element.disabled && element.getClientRects().length > 0;
                });
            }

            function setCatalogOpen(open, restoreFocus) {
                if (!catalogLayer || !catalogMobile.matches) return;
                catalogLayer.classList.toggle('is-mobile-open', open);
                document.body.classList.toggle('is-home-filter-open', open);
                catalogFilter.setAttribute('aria-hidden', open ? 'false' : 'true');
                catalogFilter.toggleAttribute('inert', !open);
                if (catalogOpen) catalogOpen.setAttribute('aria-expanded', open ? 'true' : 'false');
                if (open) {
                    catalogLastFocus = document.activeElement;
                    window.requestAnimationFrame(function () {
                        var focusable = catalogFocusable();
                        if (focusable.length) focusable[0].focus();
                    });
                } else if (restoreFocus !== false && catalogLastFocus && typeof catalogLastFocus.focus === 'function') {
                    catalogLastFocus.focus();
                }
            }

            function syncCatalogMode() {
                if (!catalogLayer) return;
                if (catalogMobile.matches) {
                    catalogFilter.setAttribute('role', 'dialog');
                    catalogFilter.setAttribute('aria-modal', 'true');
                    if (!catalogLayer.classList.contains('is-mobile-open')) {
                        catalogFilter.setAttribute('aria-hidden', 'true');
                        catalogFilter.setAttribute('inert', '');
                    }
                } else {
                    catalogLayer.classList.remove('is-mobile-open');
                    document.body.classList.remove('is-home-filter-open');
                    catalogFilter.removeAttribute('role');
                    catalogFilter.removeAttribute('aria-modal');
                    catalogFilter.removeAttribute('aria-hidden');
                    catalogFilter.removeAttribute('inert');
                    if (catalogOpen) catalogOpen.setAttribute('aria-expanded', 'false');
                }
            }
            ...
            Array.prototype.forEach.call(catalogClose, function (button) {
                button.addEventListener('click', function () { setCatalogOpen(false); });
            });
            document.addEventListener('keydown', function (event) {
                if (!catalogLayer || !catalogLayer.classList.contains('is-mobile-open')) return;
                if (event.key === 'Escape') {
                    event.preventDefault();
            ...
            if (catalogSwipe) {
                function moveCatalogSwipe(clientY) {
                    if (catalogDragging && clientY - catalogStartY > 70) {
                        catalogDragging = false;
                        setCatalogOpen(false);
                    }
                }
                catalogSwipe.addEventListener('pointerdown', function (event) {
                    catalogStartY = event.clientY;
                    catalogDragging = true;
                    catalogSwipe.setPointerCapture(event.pointerId);
                });
                catalogSwipe.addEventListener('pointermove', function (event) {
                    moveCatalogSwipe(event.clientY);
                });
                catalogSwipe.addEventListener('pointerup', function (event) {
                    moveCatalogSwipe(event.clientY);
                    catalogDragging = false;
                });
                catalogSwipe.addEventListener('pointercancel', function () { catalogDragging = false; });
                catalogSwipe.addEventListener('touchstart', function (event) {
                    if (!event.touches.length) return;
                    catalogStartY = event.touches[0].clientY;
                    catalogDragging = true;
                }, { passive: true });
                catalogSwipe.addEventListener('touchmove', function (event) {
                    if (!event.touches.length) return;
                    moveCatalogSwipe(event.touches[0].clientY);
                }, { passive: true });
                catalogSwipe.addEventListener('mousedown', function (event) {
                    catalogStartY = event.clientY;
                    catalogDragging = true;
                });
                window.addEventListener('mousemove', function (event) { moveCatalogSwipe(event.clientY); });
                window.addEventListener('mouseup', function () { catalogDragging = false; });
            }
            if (typeof catalogMobile.addEventListener === 'function') catalogMobile.addEventListener('change', syncCatalogMode);
            else catalogMobile.addListener(syncCatalogMode);
            syncCatalogMode();

            var catalogResult = catalogFilter.querySelector('[data-home-catalog-result]');
            var catalogIndexNode = catalogFilter.querySelector('[data-home-catalog-index]');
            var catalogIndex = [];
            try {
                catalogIndex = JSON.parse(catalogIndexNode ? catalogIndexNode.textContent : '[]');
            } catch (error) {
                catalogIndex = [];
            }

            function filterNumber(value, decimal) {
            ...
            function catalogCountLabel(count) {
                return count + '\u00a0' + window.msPlural(count, 'квартиру', 'квартиры', 'квартир');
            }

            function updateCatalogCount() {
                if (!catalogResult || !Array.isArray(catalogIndex)) return;
                var selectedRooms = Array.prototype.slice.call(catalogFilter.querySelectorAll('input[name="rooms[]"]:checked')).map(function (input) { return input.value; });
                var project = catalogFilter.elements.project.value;
                var floorFrom = filterFloor(catalogFilter.elements.floor_from.value);
                var floorTo = filterFloor(catalogFilter.elements.floor_to.value) || Number.POSITIVE_INFINITY;
                var priceFrom = filterNumber(catalogFilter.elements.price_from.value, false);
                var priceTo = filterNumber(catalogFilter.elements.price_to.value, false) || Number.POSITIVE_INFINITY;
                var areaFrom = filterNumber(catalogFilter.elements.area_from.value, true);
                var areaTo = filterNumber(catalogFilter.elements.area_to.value, true) || Number.POSITIVE_INFINITY;
                var queue = catalogFilter.elements.queue.value;
                var count = window.msFilterApartmentIndex(catalogIndex, {
                    project: project,
                    rooms: selectedRooms,
                    floorFrom: floorFrom,
                    floorTo: floorTo,
                ...
                catalogResult.textContent = catalogCountLabel(count);
            }

            catalogFilter.addEventListener('input', updateCatalogCount);
            catalogFilter.addEventListener('change', updateCatalogCount);
            catalogFilter.addEventListener('reset', function () { window.setTimeout(updateCatalogCount, 0); });
            updateCatalogCount();
        }

        var offers = document.querySelector('[data-home-offers]');
        if (offers) {
            var rail = offers.querySelector('[data-offers-rail]');
            function moveOffers(direction) {
                var card = rail.querySelector('.offers__card');
                if (!card) return;
                rail.scrollBy({ left: direction * (card.getBoundingClientRect().width + 20), behavior: 'smooth' });
            }
            var offersPrev = offers.querySelector('[data-offers-prev]');
            var offersNext = offers.querySelector('[data-offers-next]');
            offersPrev.addEventListener('click', function () { moveOffers(-1); });
            offersNext.addEventListener('click', function () { moveOffers(1); });
            window.msArrowEdges(rail, offersPrev, offersNext);
        }
    });
})();
...
/* End */
;
; /* Start:"a:4:{s:4:"full";s:62:"/local/templates/masterstroy/assets/js/about.js?17867043921481";s:6:"source";s:47:"/local/templates/masterstroy/assets/js/about.js";s:3:"min";s:0:"";s:3:"map";s:0:"";}"*/
(function () {
    'use strict';

    document.addEventListener('DOMContentLoaded', function () {
        var dialog = document.querySelector('[data-about-film-dialog]');
        var openButton = document.querySelector('[data-about-film-open]');
        if (!dialog || !openButton || typeof dialog.showModal !== 'function') return;

        var frame = dialog.querySelector('[data-about-film-frame]');
        var video = dialog.querySelector('[data-about-film-video]');
        var closeButton = dialog.querySelector('[data-about-film-close]');
...
/* End */
;
; /* Start:"a:4:{s:4:"full";s:61:"/local/templates/masterstroy/assets/js/news.js?17867043923152";s:6:"source";s:46:"/local/templates/masterstroy/assets/js/news.js";s:3:"min";s:0:"";s:3:"map";s:0:"";}"*/
(function () {
    'use strict';

    var init = function () {
        document.querySelectorAll('[data-news-listing]').forEach(function (listing) {
            var button = listing.querySelector('[data-news-show-all]');
            var extraCards = Array.prototype.slice.call(listing.querySelectorAll('[data-news-extra]'));
            if (!button || extraCards.length === 0) return;

            extraCards.forEach(function (card) { card.hidden = true; });
            button.hidden = false;

            button.addEventListener('click', function () {
                extraCards.forEach(function (card) { card.hidden = false; });
                button.setAttribute('aria-expanded', 'true');
                button.hidden = true;

                var firstLink = extraCards[0].querySelector('a');
        ...
        document.querySelectorAll('[data-news-gallery]').forEach(function (gallery) {
            var viewport = gallery.querySelector('[data-news-gallery-viewport]');
            var track = gallery.querySelector('.news-detail__gallery-track');
            var previous = gallery.querySelector('[data-common-prev]');
            var next = gallery.querySelector('[data-common-next]');
            if (!viewport || !track || !previous || !next) return;

            var reducedMotion = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
            var step = function () {
                var card = track.querySelector('.news-detail__gallery-card');
            ...
            if (window.msArrowEdges) {
                var syncEdges = window.msArrowEdges(viewport, previous, next);
                if (syncEdges) {
                    window.requestAnimationFrame(syncEdges);
                    window.addEventListener('load', syncEdges, { once: true });
                    track.querySelectorAll('img').forEach(function (image) {
                        if (!image.complete) image.addEventListener('load', syncEdges, { once: true });
                    });
                }
            }
        });
...
; /* /local/templates/masterstroy/assets/js/home.js?178670439216433*/
; /* /local/templates/masterstroy/assets/js/common-blocks.js?17880152709262*/
; /* /local/templates/masterstroy/assets/js/about.js?17867043921481*/
; /* /local/templates/masterstroy/assets/js/news.js?17867043923152*/

...
function useTilt(enabled: boolean) {
  ...
  return {
    onPointerMove: (event: PointerEvent<HTMLElement>) => {
      ...
      cancelAnimationFrame(frame.current)
      frame.current = requestAnimationFrame(() => { element.style.setProperty('--tilt-x', `${-y * 5}deg`); element.style.setProperty('--tilt-y', `${x * 5}deg`) })
    },
...
function ProjectCard({ project, index, onOpen, motion }: { project: CaseStudy; index: number; onOpen: (p: CaseStudy) => void; motion: boolean }) {
  ...
  return <article className={`project-card ${project.id === 'masterstroy' ? 'project-card-featured' : ''}`}>
    <button className="art-button" onClick={() => onOpen(project)} aria-label={`${project.title}: ${project.demo ? 'смотреть результат' : 'открыть кейс'}`} {...tilt}><ProjectArtwork project={project}/></button>
    <div className="project-summary"><div><div className="project-meta"><span className="eyebrow">{project.kind}</span><span className="project-number">{String(index + 1).padStart(2, '0')}</span></div><h3><button onClick={() => onOpen(project)}>{project.title} <span>↗</span></button></h3><p>{project.summary}</p><div className="stack-tags">{project.stack.slice(0, 4).map(tech => <span key={tech}>{tech}</span>)}</div></div>{project.id === 'masterstroy' && <button className="button dark" onClick={() => onOpen(project)}>Смотреть результат <span>↗</span></button>}</div>
  </article>
}
...
function WebsitePreview({ project }: { project: CaseStudy }) {
  ...
  const src = snapshot ? `${import.meta.env.BASE_URL}demos/masterstroy/${page}` : project.website!
  useEffect(() => {
    setLoaded(false); setSlow(false)
    const timer = window.setTimeout(() => setSlow(true), 12000)
    return () => clearTimeout(timer)
  }, [src, reload])
  return <div className="website-preview"><div className="preview-note"><span className={`status-dot ${snapshot ? 'snapshot-dot' : ''}`}/><span>{snapshot ? 'Демонстрационная копия публичных страниц · 09.09.2026. Формы отключены, цены справочные.' : 'Работающая публичная игра · можно играть прямо здесь.'}</span></div>{snapshot && <div className="preview-pages" role="group" aria-label="Страницы Мастерстрой">{masterPages.map(([path, label]) => <button key={path} onClick={() => { setPage(path); setReload(n => n + 1) }}>{label}</button>)}</div>}<div className="browser-toolbar"><span className="browser-dots" aria-hidden="true">● ● ●</span><span className="browser-address">{snapshot ? 'Мастерстрой / демо' : 'doonyfreeman.github.io / Wordle'}</span><div className="viewport-controls" role="group" aria-label="Ширина предпросмотра"><button onClick={() => setMobile(false)} aria-pressed={!mobile} title="Полная ширина">Десктоп</button><button onClick={() => setMobile(true)} aria-pressed={mobile} title="Ширина телефона">Мобильный</button></div><button className="reload-button" onClick={() => setReload(n => n + 1)} aria-label="Перезагрузить демонстрацию">↻</button></div><div className={`iframe-stage ${mobile ? 'mobile-preview' : ''}`} aria-busy={!loaded}>{!loaded && <div className="iframe-loading" role="status">{slow ? 'Загрузка занимает больше времени. Попробуйте обновить демонстрацию.' : 'Открываю проект…'}</div>}<iframe key={`${src}-${reload}`} src={src} title={`${project.title} — ${snapshot ? 'демонстрационные страницы' : 'живая игра'}`} sandbox={snapshot ? 'allow-scripts' : 'allow-scripts allow-same-origin'} referrerPolicy="no-referrer" onLoad={() => setLoaded(true)} onError={() => { setLoaded(true); setSlow(true) }}/></div><p className="demo-footnote">{snapshot ? 'Копия показывает публичную вёрстку. Серверные функции, карты и отправка заявок здесь не подключены. Фото загружаются с сайта проекта.' : 'Игра загружается с GitHub Pages. Если окно пустое, попробуйте перезагрузить демонстрацию.'}{slow && !loaded && ' Внешний сайт может быть временно недоступен.'}</p></div>
}
